...
const Cart = () => {
  ...
  const [productupdateDatail, setProductupdateDatail]=useState(null);
  ...
  useEffect(()=>{
    if(productupdateDatail !== null){
    dispatch(updateCartProduct({cartItemId:productupdateDatail?.cartItemId, quantity:productupdateDatail?.quantity}))
    setTimeout(()=>{
     dispatch(getUserCart())
  }, 800)
    }
                ...
                <div className="cart-col-3 d-flex align-items-center gap-15">
                  <div>
                    <input
                      ...
                      max={10}
                      id=""
                      value={productupdateDatail?.quantity ? productupdateDatail?.quantity : item?.quantity}
                      onChange={(e)=>{setProductupdateDatail({cartItemId:item?._id,quantity:e.target.value})}}
                    />
                  </div>
                  <div>
                    <AiFillDelete onClick={()=>{deletedACartProduct(item?._id)}} className="text-danger " />
                  </div>
                </div>
